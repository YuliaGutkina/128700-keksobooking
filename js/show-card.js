...
(function () {
  ...
  window.showCard = function (ad) {
    ...
    offerDialog.querySelector('.lodge__price').textContent = '' + offer.price + '₽/ночь';
    offerDialog.querySelector('.lodge__type').textContent = translateLodgeType(offer.type);
    offerDialog.querySelector('.lodge__rooms-and-guests').textContent = 'Для ' + offer.guests + ' гостей в ' + offer.rooms + ' комнатах';
    offerDialog.querySelector('.lodge__checkin-time').textContent = 'Заезд после ' + offer.checkin + ', выезд до ' + offer.checkout;
    offerDialog.querySelector('.lodge__features').textContent = '';
